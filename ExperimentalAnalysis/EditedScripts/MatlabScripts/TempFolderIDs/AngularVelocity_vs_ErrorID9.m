...
vels_cell_slow_ID_9 = cell(length(Polh_Fields), 3);
integer = 0;
...
% for trialnum = 2
   pol_dynamic = [string(Polh_Fields(trialnum))] ;
    ...
    elseif newStr == newSubstr
        ...
        more_rowsToDelete = x_pol > (x_pol(1)+900);
        ...
        for i = 1:length(pol_millis)-1
            v(i) = abs((sgf(i+1)-sgf(i))/(pol_millis(i+1)-pol_millis(i)) * 900000);
        end
        
        length_v_half = round(length(v)/2);
        
        max_inst_vel = find(v==max(v(length_v_half:end)));
        start_ind = max_inst_vel - 90;
        ...
        vels_cell_slow_ID_9{trialnum, 1}  = pol_dynamic;
        vels_cell_slow_ID_9{trialnum,2} = avg_vel;
        vels_cell_slow_ID_9{trialnum,3} = rmse;
...
%% just plot
figure(1)
avg_vel_tot_slow = vels_cell_slow_ID_9(:,2);
rmse_tot_slow = vels_cell_slow_ID_9(:,3);
...
%% edit ID number here !!
vels_cell_medium_ID_9 = cell(length(Polh_Fields), 3);
integer = 0;
for trialnum = 1:length(Polh_Fields)
...
% for trialnum = 2
    ...
    elseif newStr == newSubstr
        ...
        vels_cell_medium_ID_9{trialnum, 1}  = pol_dynamic;
        vels_cell_medium_ID_9{trialnum,2} = avg_vel;
        vels_cell_medium_ID_9{trialnum,3} = rmse;
        catch me
            fprintf('couldnt get data there bud')
        end
%         figure(trialnum)
%         plot(holo_filtered(:,1), holo_filtered(:,2) )
%         hold on
...
%% just plot
figure(1)
avg_vel_tot_medium = vels_cell_medium_ID_9(:,2);
rmse_tot_medium = vels_cell_medium_ID_9(:,3);
...
vels_cell_fast_ID_9 = cell(length(Polh_Fields), 3);
integer = 0;
for trialnum = 1:length(Polh_Fields)

    
    if trialnum < length(Holo_Fields)
    pol_dynamic = [string(Polh_Fields(trialnum + 2 - (integer) ))] 
    holo_dynamic = [string(Holo_Fields(trialnum ))]
    newStr = erase(pol_dynamic,'_POLGroundTruth');
    newSubstr = erase(holo_dynamic, '_HoloData');
    ...
    elseif newStr == newSubstr
        ...
        vels_cell_fast_ID_9{trialnum, 1}  = pol_dynamic;
        vels_cell_fast_ID_9{trialnum,2} = avg_vel;
        vels_cell_fast_ID_9{trialnum,3} = rmse;
...
%% just plot
figure(1)
avg_vel_tot_fast = vels_cell_fast_ID_9(:,2);
rmse_tot_fast = vels_cell_fast_ID_9(:,3);
...
%%
slow_ID_9 = 'VelSlow_ID_9';
medium_ID_9 = 'VelMedium_ID_9';
fast_ID_9 = 'VelFast_ID_9';
VelErrorData9.(slow_ID_9) = cell2table(vels_cell_slow_ID_9);
VelErrorData9.(medium_ID_9) = cell2table(vels_cell_medium_ID_9) ;
VelErrorData9.(fast_ID_9) = cell2table(vels_cell_fast_ID_9);
save('VelErrorData9', 'VelErrorData9')
