...
calibration_term_slow = 0;
calibration_term_medium = 0;
calibration_term_fast = 0;
%% slow
...
vels_cell_slow_ID_6 = cell(length(Polh_Fields), 3);
integer = 0;
...
% for trialnum = 2
   pol_dynamic = [string(Polh_Fields(trialnum - integer ))] ;
    ...
    elseif newStr == newSubstr
        ...
        more_rowsToDelete = x_pol > (x_pol(1)+600);
        ...
        for i = 1:length(pol_millis)-1
            v(i) = abs((sgf(i+1)-sgf(i))/(pol_millis(i+1)-pol_millis(i)) * 600000);
        end
        
        length_v_half = round(length(v)/2);
        
        max_inst_vel = find(v==max(v(length_v_half:end)));
        start_ind = max_inst_vel - 60;
        ...
        vels_cell_slow_ID_6{trialnum, 1}  = pol_dynamic;
        vels_cell_slow_ID_6{trialnum,2} = avg_vel;
        vels_cell_slow_ID_6{trialnum,3} = rmse;
...
%% just plot
figure(1)
avg_vel_tot_slow = vels_cell_slow_ID_6(:,2);
rmse_tot_slow = vels_cell_slow_ID_6(:,3);
...
%% edit ID number here !!
vels_cell_medium_ID_6 = cell(length(Polh_Fields), 3);
integer = 0;
for trialnum = 1:length(Polh_Fields)
...
% for trialnum = 2
    ...
    elseif newStr == newSubstr
        ...
        vels_cell_medium_ID_6{trialnum, 1}  = pol_dynamic;
        vels_cell_medium_ID_6{trialnum,2} = avg_vel;
        vels_cell_medium_ID_6{trialnum,3} = rmse;
...
%% just plot
figure(1)
avg_vel_tot_medium = vels_cell_medium_ID_6(:,2);
rmse_tot_medium = vels_cell_medium_ID_6(:,3);
...
try
        ...
        v = zeros(length(pol_millis),1) ;
        for i = 1:length(pol_millis)-1
            v_temp = abs((sgf(i+1)-sgf(i))/(pol_millis(i+1)-pol_millis(i)) * 600000);
            if isinf(v_temp)
                v_temp = 0;
            end
            v(i) = v_temp ;
        ...
        vels_cell_fast_ID_6{trialnum, 1}  = pol_dynamic;
        vels_cell_fast_ID_6{trialnum,2} = avg_vel;
        vels_cell_fast_ID_6{trialnum,3} = rmse;
...
%% just plot
figure(1)
avg_vel_tot_fast = vels_cell_fast_ID_6(:,2);
rmse_tot_fast = vels_cell_fast_ID_6(:,3);
...
%%
slow_ID_6 = 'VelSlow_ID_6';
medium_ID_6 = 'VelMedium_ID_6';
fast_ID_6 = 'VelFast_ID_6';
VelErrorData6.(slow_ID_6) = cell2table(vels_cell_slow_ID_6);
VelErrorData6.(medium_ID_6) = cell2table(vels_cell_medium_ID_6) ;
VelErrorData6.(fast_ID_6) = cell2table(vels_cell_fast_ID_6);
save('VelErrorData6', 'VelErrorData6')
