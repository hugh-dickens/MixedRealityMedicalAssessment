...
%             pol_comp = pol_dataframe(end-270:end, :);
%         else
%             avg_vel = 0;
%             pol_comp = [0 0];
...
%         
        holo_comp_length = length(holo_data_comp);
        pol_comp_length = length(pol_dataframe(:,1));
        ...
        for n = 1:holo_comp_length
            i= i + 1;
            if i == 1
                pol_binned_data(i) = mean(pol_dataframe(1:(n)*bins,2));
            else
                pol_binned_data(i) = mean(pol_dataframe(bins*(n-1):(n)*bins,2));
            end
        end
        

        comparing_diff = abs(pol_binned_data(:) - holo_data_comp(:,2));
...
% save('VelErrorData1', 'VelErrorData1')
