...
class UdpComms():
    def __init__(self,udpIP = "192.168.1.139",portTX =8000,portRX = 9995,enableRX=True,suppressWarnings=True):
        """
        Constructor
        :param udpIP: Hololens IP must be string e.g. "192.168.1.139"
        :param portTX: integer number e.g. 8000. Port to transmit from i.e From Python to other application
        :param portRX: integer number e.g. 9995. Port to receive on i.e. From other application to Python
        ...
        if enableRX:
            import threading
            self.rxThread = threading.Thread(target=self.ReadUdpThreadFunc)# used to be ,daemon = True)
            self.rxThread.start()
    ...
    def ReceiveData(self):
        ...
        data = None
        # self.udpSock.bind((self.udpIP, self.udpRcvPort))
        self.udpSock.bind(("", self.udpRcvPort))
        try:
...
if __name__ == '__main__':
   udpcommunications = UdpComms()
   udpcommunications.SendData("hello holo!")
#    t = udpcommunications.ReceiveData()
#    print(t)
#    UdpComms.ReceiveData() 